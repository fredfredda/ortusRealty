...
const getImportantInfrestrucutreFromDb = async (neighborhoodId) => {
    try {
        const results = await db.query(`select *, infrastructure_types.infrastructure_type from important_facilities join infrastructure_types on important_facilities.infrastructure_type_id = infrastructure_types.id where nbhd_id = ${neighborhoodId};`);
        const data = results.rows;
        return data;
    } catch (error) {
        console.log(error);
...
const getPropertyInquiriesFromDb = async (userId) => {
    try {
        const results = await db.query(`select * from property_inquiries where user_id = ${userId};`);
        const data = results.rows;
        return data;
    } catch (error) {
        console.log(error);
...
const getPropertyInquiry = async (userId, propertyId) => {
    try {
        const result = await db.query(`select * from property_inquiries where user_id = ${userId} and property_id = ${propertyId};`);
...
const createPropertyInquiry = async (userId, propertyId, defaultStatus, createdAt) => {
    try {
        await db.query(`insert into property_inquiries (prpty_id, inquiry_status_id, user_id, created_at) values (${propertyId}, ${defaultStatus}, ${userId}, '${createdAt}');`);
...
const updatePropertyInquiryStatus = async (userId, propertyId, status) => {
    try {
        await db.query(`update property_inquiries set inquiry_status_id = ${status} where user_id = ${userId} and prpty_id = ${propertyId};`);
...
const getSavedPropertiesFromdb = async (userId) => {
    try {
        const result = await db.query(`select * from saved_properties where user_id = ${userId};`);
        let data = result.rows;
        return data;
    } catch (error) {
        console.log(error);
...
const getSavedProperty = async (userId, propertyId) => {
    try {
        const result = await db.query(`select * from saved_properties where user_id = ${userId} and property_id = ${propertyId};`);
        let data = result.rows;
        return data;
    } catch (error) {
        console.log(error);
...
const createSavedProperty = async (userId, propertyId) => {
    try {
        await db.query(`insert into saved_properties (user_id, property_id) values(${userId}, ${propertyId});`);
...
const deleteSavedProperty = async (userId, propertyId) => {
    try {
        await db.query(`delete from saved_properties where user_id = ${userId} and property_id = ${propertyId};`);
...
export { getPropertyInquiry, createPropertyInquiry, updatePropertyInquiryStatus, getSavedProperty, createSavedProperty, deleteSavedProperty, getPropertyInquiriesFromDb, getSavedPropertiesFromdb, getAllPropertiesFromDb, getPropertyFromDb, getImportantInfrestrucutreFromDb, getFeaturedPropertiesFromDb, getNearbyPropertiesFromDb }